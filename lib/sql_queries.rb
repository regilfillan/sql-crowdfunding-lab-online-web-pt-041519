...
"SELECT users.name, users.age, SUM(pledges.amount) FROM users LEFT JOIN pledges ON users.id = pledges.user_id GROUP BY users.name ORDER BY users.name ASC;"
end
...
# FROM projects 
# LEFT JOIN pledges ON projects.id = pledges.project_id 
# WHERE pledges.amount > projects.funding_goal OR pledges.amount = projects.funding_goal;"
end
...
FROM pledges
LEFT JOIN projects ON projects.id = pledges.project_id
